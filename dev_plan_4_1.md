...
- ~~Co-op before a complete solo game~~ **(REVISED, see Block N):** co-op *content* still comes late,
  ...
  modifier layer (demand ×, cost ×) on top of solo balance, never a separate balance table.
...
- ✅ A3 Money & HUD (`MoneySystem`, `MoneyHUD`) — *v4: wallet becomes SHARED + host-authoritative in
  N3 (N0-c); HUD unchanged*
- ✅ A4 Items & inventory (`ItemSO`, `ItemCategory`, `Inventory`) — *v4 decision: inventories are
...
- ✅ A7 NavMesh + `NavMeshObstacle` (carve) on the player — *v4: same carve for every `NetworkPlayer`*
- ✅ A8 First NPC & first sale (`NPCVisitor` state machine) — **first full loop**
- ✅ A9 Visitor flow (`NPCSpawner`, `NPCTypeSO`) — *v4: spawner will consume B3's new open
  multipliers (rep, player count) — no rewrite, new inputs (N0-e)*
- ✅ A10 Saving (`SaveSystem`, `SaveData`, JSON in `persistentDataPath`) — *v4: in co-op the save is
...
- ✅ B3 NPC traffic by hour (density curve: morning low, midday peak, night ~0) — *v4 EXTENSION
  ...
  ×~1.6–1.7 for 2P). Both shown in Evening Summary ("visitors: base 20 × rep 1.3 × duo 1.6") —
  still no hidden buffs, the multipliers are printed*
- ✅ B4 Seasons (`SeasonManager` — 4 seasons, sky tint, supplier availability) — *v4 config: season
...
- ✅ B8 NPC purchase rules without hidden demand (concrete refusal reasons in logs) — *v4: this
...
### C5. PauseMenu `[assets: ready]` ✅ *(v4 REVISION REQUIRED — N0-b)*
...
top unmet demand (teaser line from the Demand Journal — "cheese was asked for 3× today"). Design it
...
concrete refusal reasons — reuse that pipe), the item is tallied: "cheese: asked 14× this week".
...
**Visual revival is the reward:** each ★ / story milestone physically changes the market — empty
...
**Check:** reach 2★ → a new supplier/decor unlocks, a boarded-up stall visibly opens, and the player
...
- **N0-e Traffic (B3 + A9):** density curve × reputation tier (D9) × player count (J1) — both
...
**Check:** cheese asked 15+× over 2 weeks → innkeeper spawns a contract dialogue; accept → weekly
...
**Do:** visitor flow and per-item demand capacity ×~1.6–1.7 for the 2nd player (sub-linear ON
PURPOSE: two players produce 2×, so the per-item ceiling must arrive *earlier* per head — pushing
the pair into different professions instead of double tomatoes). Big build costs (greenhouse,
second stall, story milestones) ×~1.4–1.5. All scaled numbers remain visible in the journal —
...
### Block A — Playable skeleton
- [x] A1 Walking · [x] A2 Look & prompt · [x] A3 Money & HUD *(→N0-c)* · [x] A4 Items & inventory *(→N0-c)*
- [x] A5 Supplier (debug) · [x] A6 Stall (debug) · [x] A7 NavMesh + obstacle
- [x] A8 First NPC & sale · [x] A9 Visitor flow *(→N0-e)* · [x] A10 Saving *(→N0-f)*

### Block B — Stable market, no hidden coefficients
- [x] B1 Game time *(→N0-a)* · [x] B2 Daylight · [x] B3 Traffic by hour *(→N0-e)* · [x] B4 Seasons *(14d cfg)*
...
- [ ] N0 Retrofit audit of v3 systems (a time/day-end · b pause · c money/inventory · d stall lock
      · e traffic multipliers · f saves · g debugger)
- [ ] N1 Netcode bootstrap · [ ] N2 Second player · [ ] N3 Shared authoritative state (shared wallet)
- [ ] N4 Asymmetric sessions · [ ] N5 Personal attribution

### Block E — Farm
- [x] E1 CropPlot + CropSO *(state machine will grow: E4/E10/N4)* · [ ] E2 Visual stages · [ ] E3 Seasonality · [ ] E4 Quality (stars, care)
- [ ] E5 Cost of production & crop roles · [ ] E6 Greenhouse · [ ] E7 Beehive · [ ] E8 Flowers & bouquets
- [ ] E9 Farm tutorial · [ ] E10 Soil fatigue · [ ] E11 Tool ladder → drip · [ ] E12 Kitchen & preserving
...
- [ ] H1 CraftingStation base · [ ] H2 Bakery · [ ] H3 Brewery · [ ] H4 Smithy (cut?)
- [ ] H5 Apothecary (cut?) · [ ] H6 Tailor (cut?) · [ ] H7 Recipes as finds · [ ] H8 Unique recipe

### Block I — Social systems & town
- [ ] I1 NPC dialogue & relationships · [ ] I2 Contracts (supply + sales) · [ ] I3 Town licenses
- [ ] I4 Reputation in action

### Block J — Co-op balance & experience
- [ ] J1 Demand & cost scaling · [ ] J2 Role split / WorkerNPC parity · [ ] J3 Hire-point check
- [ ] J4 Journal as coordination board · [ ] J5 Goal scaling & pair sync
