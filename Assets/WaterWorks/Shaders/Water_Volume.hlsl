...
// The blit source is bound by the Water_Volume renderer feature as _BlitTexture (Blit.hlsl),
// so the package's own _MainTex slot is no longer declared here.
...
float3 SampleSceneColour(float2 uv)
{
...
SurfaceDescription SurfaceDescriptionFunction(SurfaceDescriptionInputs IN)
{
    ...
    // The box follows the camera horizontally so a small volume can cover an endless ocean.
    float3 volumePos = pos;
    volumePos.xz += _WorldSpaceCameraPos.xz;

    float3 posBL = volumePos - bounds / 2;
    float3 posTR = volumePos + bounds / 2;
    ...
    {
        surface.BaseColor = SampleSceneColour(IN.ScreenPosition.xy / IN.ScreenPosition.w);
        return surface;
    }
    ...
    UNITY_LOOP
        for (i = 0; i < 250 && distTravelled + distToBox < depth && distTravelled < distInBox; i++)
        {
            ...
            float yPos = rayPos.y - volumePos.y;
            yPos += bounds.y / 2;
            yPos /= bounds.y;
            ...
            // _Time is a float4; the original relied on the implicit truncation to .x, kept as-is.
            float noiseValue = Cellular(rayPos.xz * 0.2, (_Time.x * 50));
    ...
    float3 background = SampleSceneColour((IN.ScreenPosition.xy / IN.ScreenPosition.w) + refraction);
