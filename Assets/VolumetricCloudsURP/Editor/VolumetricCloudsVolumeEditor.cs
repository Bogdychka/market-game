...
class VolumetricCloudsEditor : VolumeComponentEditor
{
    ...
    {
        // Guards against a Unity 6 Play Mode timing issue where this editor's target volume
        // component is momentarily null, which throws SerializedObjectNotCreatableException from
        // inside serializedObject/base.OnEnable() before any of our own code runs (same class of
        // bug as the sibling PhysicallyBasedSkyURP package's upstream issue #6).
        if (target == null)
            return;
