...
Historical release notes moved out of CHANGELOG.md to keep active handoffs small.
...
- Stall could not be opened: the MarketStall's only interaction collider was a thin 0.2m-tall counter
  ...
  "Manage stall" prompt reliably appears at eye level. Confirmed via game.log (previously zero
...
- C4: Stall placement price inputs now warn when the entered price is below `ItemSO.BaseBuyPrice` by
  coloring the TMP input red and showing "< buy price"; placement remains allowed at any positive
  price. (Codex)
