...
const requestTimeoutGraceSeconds = 5;
const paramsSchema = z.object({
  ...
  timeoutSeconds: z.number().positive().optional().describe('Timeout for this test run in seconds. Overrides the global MCP request timeout for this run only (optional)')
});
...
async function toolHandler(mcpUnity: McpUnity, params: any = {}): Promise<CallToolResult> {
  ...
  const response = await mcpUnity.sendRequest({
    ...
    timeout: getRequestTimeoutMs(timeoutSeconds)
  });
...
function getRequestTimeoutMs(timeoutSeconds?: number): number | undefined {
  if (timeoutSeconds === undefined) {
    return undefined;
  }

  return Math.ceil(timeoutSeconds + requestTimeoutGraceSeconds) * 1000;
}
