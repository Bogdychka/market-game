...
async function resourceHandler(mcpUnity: McpUnity): Promise<ReadResourceResult> {
      ...
      const manifestPackages = response.manifestPackages || [];
      const resolvedPackages = response.resolvedPackages || [];
      ...
      const packagesData = {
        ...
        packagesLockPath: response.packagesLockPath,
        projectPackages: projectPackages.map(mapPackage),
        registryPackages: registryPackages.map(mapPackage),
        manifestPackages: manifestPackages.map(mapPackage),
        resolvedPackages: resolvedPackages.map(mapPackage)
      };
