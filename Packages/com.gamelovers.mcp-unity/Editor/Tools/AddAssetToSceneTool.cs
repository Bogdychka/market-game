        ...
        public override JObject Execute(JObject parameters)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(parentPath) || parentId.HasValue)
                {
                    ...
                    {
                        parent = McpUnity.Utils.ObjectIdUtils.ObjectFromLegacyInstanceId(parentId.Value) as GameObject;
                    }
            ...
            return new JObject
            {
                ["success"] = true,
                ["type"] = "text",
                ["message"] = $"Successfully added asset '{asset.name}' with instance ID {McpUnity.Utils.ObjectIdUtils.GetLegacyInstanceId(instance)} to the scene",
                ["instanceId"] = McpUnity.Utils.ObjectIdUtils.GetLegacyInstanceId(instance)
            };
        }
    }
}
