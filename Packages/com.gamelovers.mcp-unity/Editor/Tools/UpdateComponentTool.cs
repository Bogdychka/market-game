...
namespace McpUnity.Tools
{
    ...
    public class UpdateComponentTool : McpToolBase
    {
        ...
        private object ConvertJTokenToValue(JToken token, Type targetType)
        {
            ...
            // Handle UnityEngine.Object and derived scene/asset references.
            if (typeof(UnityEngine.Object).IsAssignableFrom(targetType))
            {
                return ConvertJTokenToUnityObject(token, targetType);
            }
        ...
        private UnityEngine.Object ConvertJTokenToUnityObject(JToken token, Type targetType)
        {
            if (token.Type == JTokenType.Integer)
            {
                UnityEngine.Object obj = ObjectIdUtils.ObjectFromLegacyInstanceId(token.ToObject<int>()) as UnityEngine.Object;
                return CoerceUnityObject(obj, targetType);
            }
            ...
            if (token.Type != JTokenType.String)
            {
                try
                {
                    return token.ToObject(targetType) as UnityEngine.Object;
                }
                catch (Exception ex)
                {
                    McpLogger.LogError($"[MCP Unity] Error converting Unity object reference to {targetType.Name}: {ex.Message}");
                    return null;
                }
            }

            string reference = token.ToObject<string>();
            if (string.IsNullOrWhiteSpace(reference))
            {
                return null;
            }

            if (int.TryParse(reference, out int instanceId))
            {
                UnityEngine.Object obj = ObjectIdUtils.ObjectFromLegacyInstanceId(instanceId) as UnityEngine.Object;
                return CoerceUnityObject(obj, targetType);
            }

            UnityEngine.Object asset = TryLoadAssetReference(reference, targetType);
            if (asset != null)
            {
                return asset;
            }

            GameObject sceneObject = GameObject.Find(reference) ?? FindGameObjectByPath(reference);
            if (sceneObject != null)
            {
                return CoerceUnityObject(sceneObject, targetType);
            }

            return null;
        }

        private UnityEngine.Object TryLoadAssetReference(string reference, Type targetType)
        {
            ...
            UnityEngine.Object asset = AssetDatabase.LoadAssetAtPath(reference, targetType);
            return CoerceUnityObject(asset, targetType);
        }

        private UnityEngine.Object CoerceUnityObject(UnityEngine.Object obj, Type targetType)
        {
            if (obj == null)
            {
                return null;
            ...
            if (obj is GameObject gameObject && typeof(Component).IsAssignableFrom(targetType))
            {
            ...
            if (obj is Component component)
            {
                ...
                {
                    return component.GetComponent(targetType);
                }
            }
